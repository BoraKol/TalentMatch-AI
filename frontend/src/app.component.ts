...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [CommonModule, SettingsComponent, JobMatchComponent, ToastComponent, AnalyticsComponent],
  template: `
    <app-toast></app-toast>
      ...
      <aside class="w-64 bg-slate-900 text-white flex flex-col shadow-2xl z-10">
        ...
        <nav class="flex-1 p-4 space-y-2">
          <button 
            ...
            [class]="currentView() === 'dashboard' ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-white'">
          ...
          <button 
            (click)="currentView.set('match')"
            class="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors"
            [class]="currentView() === 'match' ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-white'">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <button 
            ...
            class="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors"
            [class]="currentView() === 'settings' ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-white'">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="p-4 border-t border-slate-800">
          <div class="flex items-center gap-3">
            ...
            <button class="ml-auto text-slate-500 hover:text-white">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="p-8">
            ...
            <div class="bg-blue-600 rounded-2xl p-8 text-white relative overflow-hidden shadow-xl shadow-blue-900/20">
              <div class="relative z-10">
                ...
                <button (click)="currentView.set('match')" class="bg-white text-blue-600 px-6 py-2 rounded-lg font-bold hover:bg-blue-50 transition-colors">
                  Try AI Matching Now
                </button>
              </div>
              <!-- Decorative Circle -->
...
export class AppComponent {
  currentView = signal<'dashboard' | 'match' | 'settings'>('dashboard');
}