  ...
  template: `
    <div class="min-h-screen flex items-start justify-center bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 relative overflow-auto">
      ...
      <div class="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-full pointer-events-none">
          <div class="absolute top-0 right-0 w-96 h-96 bg-emerald-100 rounded-full blur-3xl opacity-50 -translate-y-1/2 translate-x-1/3"></div>
          <div class="absolute bottom-0 left-0 w-80 h-80 bg-teal-100 rounded-full blur-3xl opacity-50 translate-y-1/3 -translate-x-1/4"></div>
      </div>
      ...
      <div class="max-w-xl w-full relative z-10">
        <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white/50 overflow-hidden">
          ...
          <div class="bg-gradient-to-r from-emerald-600 to-teal-600 p-8 text-center text-white">
            ...
            <h1 class="text-2xl font-extrabold tracking-tight">Employer Registration</h1>
            <p class="mt-2 text-emerald-100 font-medium">Find the best talent for your organization.</p>
          </div>
            ...
            <form [formGroup]="registerForm" (ngSubmit)="onSubmit()" class="space-y-6">
              ...
              <div>
                <h3 class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                   <span class="w-8 h-px bg-slate-300"></span> Your Information <span class="w-full h-px bg-slate-100"></span>
                </h3>
                <div class="space-y-4">
                  ...
                  <div class="space-y-1">
                    ...
                    <div class="relative">
                      ...
                      <input formControlName="password" [type]="showPassword() ? 'text' : 'password'" 
                             class="w-full pl-10 pr-10 rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-emerald-500 focus:ring-emerald-500 px-4 py-3 transition-all duration-200"
              ...
              <div>
                <h3 class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                   <span class="w-8 h-px bg-slate-300"></span> Company Details <span class="w-full h-px bg-slate-100"></span>
                </h3>
               <div class="space-y-4">
                  <div class="space-y-1">
              ...
              <div class="pt-2">
                <button type="submit" [disabled]="isLoading() || registerForm.invalid"
                  ...
                  {{ isLoading() ? 'Creating Your Account...' : 'Create Employer Account' }}
                </button>
              </div>
              ...
              <div class="text-center">
                  <p class="text-sm text-slate-600">
                    ...
                    <a routerLink="/login" class="font-bold text-emerald-600 hover:text-emerald-500 transition-colors">Sign in here</a>
                  </p>
              </div>
...
export class RegisterEmployerComponent {
  ...
  onSubmit() {
    if (this.registerForm.valid) {
      ...
      const url = `${environment.apiUrl}/auth/register/employer`;

      this.http.post(url, this.registerForm.value).subscribe({
        next: () => {
          this.isLoading.set(false);
          this.router.navigate(['/login'], { queryParams: { registered: 'employer' } });
        },
        error: (err) => {
          this.isLoading.set(false);
